...
<?php echo $this->Form->submit('Report', array('type' => 'button', 'value' => 'Report', 'id' => 'report')); ?>	
	...
	<tr>
		...
		<td class="actions">
			...
			<?php 
				if ($groups == 1) { // show only admin
					echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $browsing['Browsing']['id']), null, __('Are you sure you want to delete # %s?', $browsing['Browsing']['id'])); 
				}
			?>
		</td>
	</tr>
...
$('#report').click(function() {
 	var answer = confirm('The report will producing based on the search criteria. Please make sure the date range is not too long or the download will time consuming.\n\nAre you sure want to generate the browsings report?');
 	if (answer == true) {
 		var site = $('#site').val();
 		if ($('#BrowsingIcno').val()) {
 			var icno = $('#BrowsingIcno').val();
 		}
 		else {
 			var icno = 0;
 		}
 		var name = 0;
 		var datefrom = $('#BrowsingTimeStartYear').val() + '-' + $('#BrowsingTimeStartMonth').val() + '-' + $('#BrowsingTimeStartDay').val();
 		var dateto = $('#BrowsingTimeEndYear').val() + '-' + $('#BrowsingTimeEndMonth').val() + '-' + $('#BrowsingTimeEndDay').val();
 		// alert(cms_web.v1.base_url + 'certrecords/jasperbrowsings/' + icno + '/' + site + '/' + datefrom + '/' + dateto);
		window.location.href = cms_web.v1.base_url + 'certrecords/jasperuserdetails/browsings/' + site + '/' + icno + '/' + name + '/' + datefrom + '/' + dateto;
	}	  
})	
</script>